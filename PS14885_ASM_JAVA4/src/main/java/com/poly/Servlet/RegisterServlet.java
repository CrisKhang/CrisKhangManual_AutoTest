		...
		if (i == 2) {
			if (tenDN.isEmpty()) {
				...
				if (tenDN.length() < 6 || tenDN.length() > 15) {
					doFill(req, resp);
					req.setAttribute("backround", "danger");
					req.setAttribute("message", "Tên đăng nhập phải từ 6-15 kí tự!");
					req.setAttribute("show", "true");
				} else {
...
//				i++;
		...
		if (i == 4) {
			if (matKhau.isEmpty()) {
				...
				if (matKhau.length() < 7) {
					doFill(req, resp);
					req.setAttribute("backround", "danger");
					req.setAttribute("message", "Mật khẩu phải ít nhất 7 kí tự!");
					req.setAttribute("show", "true");
				} else {
					doFill(req, resp);
					i++;
		...
		if (i == 6) {
			if (dieuKhoan == null) {
				doFill(req, resp);
				req.setAttribute("backround", "danger");
				req.setAttribute("message", "Vui lòng xác nhận điều khoảng!");
				req.setAttribute("show", "true");
			} else {
				doFill(req, resp);
				i++;
...
//				
				...
				req.setAttribute("show", "true");
				resp.sendRedirect("/PS14885_ASM_JAVA4/views/login.jsp");
			} catch (Exception e) {
				req.setAttribute("backround", "danger");
